...
def parse_args():
    ...
    parser.add_argument('--img-list', type=str, default='', help='Image file list')
    parser.add_argument(
        '--target-layers',
        default=[],
        nargs='+',
    ...
    parser.add_argument(
        '--save-dir',
        type=str,
        help='The dir to save visualize cam image, default not to save.'
    )
...
def main(args, model: t.Optional[nn.Module] = None, cfg: t.Optional[object] = None):
    if model is None:
    ...
    # apply transform and perpare data, build transforms function, resize, crop, normalize
    transforms = Compose(
...
if __name__ == '__main__':
    # python .\tools\visualization\vis_cam.py .\work_dirs\resnet\spmca\resnet50_8xb32_in1k_scsa.py .\work_dirs\resnet\spmca\resnet50-spmca7\20240408_124835\best_accuracy_top1_epoch_94.pth --img-list .\work_dirs\random_pic\ --save-dir .\work_dirs\visual_pic\spmca  --target-layers backbone.layer4.2 --method GradCAM

    # python .\tools\visualization\vis_cam.py .\work_dirs\resnet\spmca\resnet50_8xb32_in1k_scsa.py
    # .\work_dirs\resnet\spmca\resnet50-spmca7\20240408_124835\best_accuracy_top1_epoch_94.pth --img-list
    # .\work_dirs\random_pic\ --save-dir .\work_dirs\visual_pic\spmca  --target-layers backbone.layer4.2 --method
    # GradCAM

    # Supports batch visualization
    args = parse_args()
    if args.img_list != '':
        cfg = Config.fromfile(args.config)
        if args.cfg_options is not None:
        ...
        groups = os.listdir(args.img_list)
        for group in groups:
            img_path_file: str = os.path.join(args.img_list, group)
            save_dir: str = os.path.join(args.save_dir, group)
            os.makedirs(save_dir, exist_ok=True)
            with open(img_path_file, 'r') as f:
                images: t.List = json.load(f)
                for img in images:
                    args.img = img
                    img_name = img.split('\\')[-1]
                    args.save_path = os.path.join(save_dir, img_name)
                    main(args, model, cfg)
            print(f'{img_path_file} processed successfully!')
    # Supports single visualization
    else:
        main(args)
